...
public class BVBTransferRequestUtil {
	...
	public BVBIBFTFundTransferRequest buildIBFTFundTransferRequest(BankTransferChildRecords childRecords,
																   BankTransferTransaction bankTransferTransaction
	) throws Exception {
		String url = configLoader.getBvbIBFTFundTransferUrl();
		BVBIBFTFundTransferRequest bvbibftFundTransferRequest
				= new BVBIBFTFundTransferRequest();
		BVBIBFTFundTransferRequestData bvbibftFundTransferRequestData
				= new BVBIBFTFundTransferRequestData();
		bvbibftFundTransferRequest.setData(bvbibftFundTransferRequestData);
		bvbibftFundTransferRequest.setRequestId(childRecords.getTransId());
		if (childRecords.getRecvAcctCard() != null) {
			bvbibftFundTransferRequest.getData().setCardNo(childRecords.getRecvAcctCard());
		} else {
			bvbibftFundTransferRequest.getData().setAccountNo(childRecords.getRecvAcctId());
		}
		bvbibftFundTransferRequest.getData().setBankCode(childRecords.getRecvBank().getVccbBenId().get(0));
		if (childRecords.getRecvBank().getCode().equals(configLoader.getBvbBankTransferOnefinBankId().get(0))) {
			bvbibftFundTransferRequest.getData().setOnus(BankConstants.BVBonus.INTERNAL.getValue());
		} else {
			bvbibftFundTransferRequest.getData().setOnus(BankConstants.BVBonus.EXTERNAL.getValue());
		}
		bvbibftFundTransferRequest.getData().setAmount(childRecords.getAmount().toString());
		bvbibftFundTransferRequest.getData().setFeeModel(bankTransferTransaction.getFeeType());
		bvbibftFundTransferRequest.getData().setDescription(childRecords.getRemark());
		bvbibftFundTransferRequest.setClientCode(bankTransferTransaction.getProviderId());
		Optional.ofNullable(bankTransferTransaction.getMerchantId())
				.ifPresent(bvbibftFundTransferRequest::setClientUserId);
		bvbibftFundTransferRequest.setTime(bankTransferTransaction.getBankTransTime());

		return bvbibftFundTransferRequest;
	}

	public BVBIBFTQueryStatusRequest buildIBFTTransferInquiryRequest(BankTransferInquiryRequest data,
																	 BankTransferTransaction queryTrans) throws Exception {

		BankTransferChildRecords bankTransferChildRecords
				= new ArrayList<>(queryTrans.getRecords()).get(0);
		BVBIBFTQueryStatusRequest bvbibftQueryStatusRequest
				= new BVBIBFTQueryStatusRequest();

		bvbibftQueryStatusRequest.setRequestId(data.getRequestId());
		bvbibftQueryStatusRequest.setClientCode(queryTrans.getProviderId());
		Optional.ofNullable(queryTrans.getMerchantId())
				.ifPresent(bvbibftQueryStatusRequest::setClientUserId);
		bvbibftQueryStatusRequest.setTime(dateTimeHelper.parseDate2(data.getTransTime(), VietinConstants.DATE_FORMAT_yyyyMMDDHHmmss));
		BVBIBFTQueryStatusRequestData dataInquiry
				= new BVBIBFTQueryStatusRequestData();
		dataInquiry.setRefRequestId(bankTransferChildRecords.getTransId());
//		dataInquiry.setDescription("");
//		dataInquiry.setExtraInfo("");
		bvbibftQueryStatusRequest.setData(dataInquiry);
