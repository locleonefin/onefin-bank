...
public class ControllerAspect {
    ...
    @Around("execution(* com.onefin.ewallet.bank.service.bvb.BVBRequestUtil.requestBVB(..))")
    public Object afterRequestBVB(ProceedingJoinPoint joinPoint) throws Throwable {

        ResponseEntity<?> bvbResponse = (ResponseEntity<?>) joinPoint.proceed();

        LOGGER.info("bvbResponse " + bvbResponse);

        com.onefin.ewallet.bank.dto.vietin.ConnResponse responseSignature = bvbRequestUtil.checkSignature(bvbResponse);

        LOGGER.info("responseSignature " + responseSignature);

        if (responseSignature != null) {
            throw new RuntimeBadRequestException("Chữ ký số không hợp lệ");
        }
